...
function _facRows(){
  ...
  _facOptList().forEach(function(o){
    rows.push(['opcion_'+o,'Ver la Opci&oacute;n '+o,'Habilita el armador de la Opci&oacute;n '+o+'. El selector aparece s&oacute;lo si tiene m&aacute;s de una habilitada.']);
  });
...
function loadPromosCatalogo(cb){
  ...
  Promise.all([
    _sb.from('promos_galicia_cache').select('id,titulo,subtitulo,imagen,fecha_hasta,tipo_promocion'),
...
function _promoBuscarCandidatos(marca,catalogo){
  ...
  var scored=catalogo.map(function(p){
    ...
    if(nTitulo===nMarca)score=100;
      ...
      // palabra completa compartida (min 4 letras, para no pescar "de","on",etc.)
      var compartidas=tokensMarca.filter(function(t){return t.length>=4&&tokensTitulo.indexOf(t)>=0;});
      if(compartidas.length>0)score=40;
    }
...
function validarPromos(){
  ...
  if(!_promosCat||!_promosCat.length){
    showToast('El catálogo todavía se está cargando, esperá un instante e intentá de nuevo');
    if(!_promosCatLoading)loadPromosCatalogo();
    return;
  }
