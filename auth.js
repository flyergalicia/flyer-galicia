...
function logFlyerToSupabase(v,fn,fmt){
  ...
  var extra={};
  if(v){
    ['m1','m2','m3','m4','nombre','celular','email','nombre2','celular2','email2'].forEach(function(k){
      if(v[k])extra[k]=v[k];
    });
  }
  ...
  query.limit(300).then(function(r){
    ...
    _loadProfileMap(function(profileMap){
      ...
      container.innerHTML=data.map(function(row){
        ...
        var montos=[ex.m1,ex.m2,ex.m3,ex.m4].filter(Boolean);
        var montosHtml=montos.length?'<div class="reg-montos">'+montos.map(function(m){return '<span class="reg-monto">'+m+'</span>';}).join('')+'</div>':'';
...
function exportRegistros(){
  ...
  query.then(function(r){
    ...
    _loadProfileMap(function(profileMap){
      ...
      ws['!cols']=[{wch:22},{wch:28},{wch:32},{wch:14},{wch:14},{wch:14},{wch:14},{wch:28},{wch:18},{wch:36},{wch:28},{wch:18},{wch:36},{wch:8}];
