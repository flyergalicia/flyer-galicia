...
function _fgParseContacto(texto){
  ...
  var cands=[],reTel=/(?:[+(]?\d[\d\s().\-/]{6,}\d)/g,m; // [+(]? también captura el "(" de apertura, para mostrar el original completo
  while((m=reTel.exec(sinMail))){
    var bruto=m[0].trim(),dig=_padDigits(bruto);
    if(dig.length<8||dig.length>15)continue;
    if(/\d{2}-\d{8}-\d/.test(bruto)){out.ignorado.push('un CUIT');continue;}
    if(/^\d{1,2}[/.]\d{1,2}[/.]\d{2,4}$/.test(bruto)){out.ignorado.push('una fecha');continue;}
    var i=sinMail.indexOf(bruto),antes=sinMail.slice(Math.max(0,i-18),i);
    cands.push({bruto:bruto,dig:dig,puntos:(_FG_PC_MOVIL.test(antes)?10:0)-(_FG_PC_FIJO.test(antes)?10:0)});
  }
  var ok=cands.map(function(c){return {bruto:c.bruto,dig:c.dig,puntos:c.puntos,t:_fgPartirTel(c.dig)};})
    .filter(function(c){return c.t;}).sort(function(a,b){return b.puntos-a.puntos;});
  if(ok.length){
    var c=ok[0];
    if(!_fgVerificaTel(c.t))out.avisos.push('no pude verificar el número — lo dejé como estaba');
    else{out.celular=_fgFmtCel(c.t);out.celOriginal=c.bruto;out.celQuitados=c.t.quitados.map(function(q){return q.etq;});}
    ok.slice(1).forEach(function(x){out.ignorado.push('otro número ('+x.bruto+')');});
  }else if(cands.length){
    out.avisos.push('"'+cands[0].bruto+'" no parece un celular argentino');
  }
  var sinTel=sinMail.replace(reTel,' ');
...
function _fgTogglePaste(n){
  ...
  if(btn)btn.classList.toggle('on',abrir);
  if(abrir){
...
function _fgApplyParsed(n,r){
  ...
  if(esPersona){
    ...
    if(!mailFinal&&r.nombre){
      mailFinal=_fgMailFromName(r.nombre);
      if(mailFinal)r.avisos.push('mail sugerido a partir del nombre (no vino en el texto)');
    }
    _setVal('email'+sfx,mailFinal||'');
    if(!mailFinal&&antesE)r.avisos.push('Mail: no encontré — lo dejé vacío');
    var mEl=document.getElementById('email'+sfx);
