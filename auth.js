...
var FLYER_CFG_DEFAULT = {
  ...
  // ENTERAS (pre + valor + post) sobre un rectángulo que TAPA la zona, así el
  // PDF limpio viene sin esas líneas y el importe puede tener cualquier largo.
  // El color del rectángulo se MUESTREA del propio flyer al lado de la zona
  // (bg es sólo el fallback): con un color fijo quedaba una franja visible
  // porque el panel del cuadro es durazno (#f7e6dc), no crema. Ancladas arriba
  // (el cuadro está en la mitad superior). x = borde izquierdo si align:left,
  // centro si align:center. Posiciones, tamaños y colores medidos sobre los
  // PDFs reales (SIEMENS combustible/supermercado, rasterizados a 1240px); el
  // admin los termina de acomodar en el calibrador.
  benef:{
    titulo:{x:380,y:2628,fs:44,mw:780,h:56,pad:24,bold:true,bg:"#f7e6dc",align:"left",color:"#fa6400",pre:"¡Beneficio exclusivo ",post:"!"},
    tope:{x:660,y:2761,fs:19,mw:560,h:28,pad:24,bold:false,bg:"#f7e6dc",align:"center",color:"#000000",pre:"Tope de reintegro mensual ",post:""}
  }
};
...
function _fgBenefMigra(B){
  if(!B)return B;
  if(B.titulo&&B.titulo.color==='#f5921e')B.titulo.color='#fa6400';
  if(B.tope&&B.tope.color==='#222222')B.tope.color='#000000';
  return B;
}
...
function _fgCfg(){
  ...
  return {imgW:c.imgW||d.imgW,imgH:c.imgH||d.imgH,
    ...
    benef:_fgBenefMigra({titulo:_fgMerge(d.benef.titulo,cb.titulo),tope:_fgMerge(d.benef.tope,cb.tope)})};
}
...
// Cada línea tapa su zona y escribe pre+valor+post; si no entra en mw se achica
// la letra (como el nombre de empresa). Sin importe, la línea del tope se tapa
// igual pero no se escribe.
// El color con que se tapa se MUESTREA del flyer (mediana de 10 píxeles justo
// arriba y abajo del rectángulo): así el parche es indistinguible del panel,
// venga del color que venga. Si el canvas no deja leer píxeles, cae en Z.bg.
...
function fgDrawBenef(c,s,v){
  ...
  function linea(Z,valor){
    ...
    var center=(Z.align==='center'),pad=Math.round((Z.pad!=null?Z.pad:24)*se);
    ...
    var txt=(Z.pre||'')+valor+(Z.post||''),peso=(Z.bold===false)?'':'bold ';
    c.font=peso+fs+"px Arial,sans-serif";
    var w=c.measureText(txt).width;
    if(w>mw&&w>0){fs=Math.max(Math.floor(fs*mw/w),Math.max(1,Math.round(8*se)));c.font=peso+fs+"px Arial,sans-serif";}
...
function _calEnsureDom(){
  ...
  st.textContent=
    ...
    '.cal-bl{display:grid;grid-template-columns:110px 1fr 1fr 64px 44px;gap:6px;align-items:center;margin-bottom:5px}'+
  ...
  m.innerHTML=
    '<div class="cal-card">'+
      ...
      '<div id="cal-benef-row">'+
        '<div class="cal-bl cal-bl-head"><span>Cartel del beneficio</span><span>Texto antes</span><span>Texto despu&eacute;s</span><span>Letra px</span><span>Color</span></div>'+
        '<div class="cal-bl"><b style="color:#f5921e">T&iacute;tulo (+ nombre)</b>'+
          ...
          '<input id="cal-bt-col" type="color" title="Color del texto" oninput="_calBenefTexto(\'titulo\',\'color\',this.value)"></div>'+
        '<div class="cal-bl"><b style="color:#b26a00">Tope (+ importe)</b>'+
          ...
          '<input id="cal-bp-col" type="color" title="Color del texto" oninput="_calBenefTexto(\'tope\',\'color\',this.value)"></div>'+
        '<div style="color:var(--gray);font-size:.64rem">Ej.: para que salga &laquo;Tope de reintegro mensual $24.000 (5)&raquo;, texto despu&eacute;s = &laquo; (5)&raquo;. El nombre y el importe los carga cada asesor. El fondo del parche se copia solo del flyer (no hay que elegirlo).</div>'+
      '</div>'+
...
function _calBenefRowSync(){
  ...
  s('cal-bt-pre',B.titulo.pre);s('cal-bt-post',B.titulo.post);s('cal-bt-fs',B.titulo.fs);s('cal-bt-col',B.titulo.color||'#000000');
  s('cal-bp-pre',B.tope.pre);s('cal-bp-post',B.tope.post);s('cal-bp-fs',B.tope.fs);s('cal-bp-col',B.tope.color||'#000000');
}
