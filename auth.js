...
function padronSearch(q,limit){
  ...
  for(var i=0;i<rows.length&&out.length<lim;i++){
    ...
    if(!hit){
      ...
      hit=toks.every(function(k){return e.indexOf(k)>=0;});
    }
    if(hit)out.push(r);
  }
  return out;
...
function _padCuitCount(rows){
  var n=0;(rows||[]).forEach(function(r){n+=(r.cuits||[]).length;});return n;
}
function loadPadron(force,cb){
  ...
  fetch(FLYERS_PUBLIC+_PADRON_FILE+'?t='+Date.now(),{cache:'no-cache'})
    .then(function(r){return r.ok?r.json():null;})
    .then(function(d){
      _padron=(d&&Array.isArray(d.rows))?d.rows:[];
...
function importPadron(input){
  ...
  reader.onload=function(e){
    try{
      ...
      var rows=[];
      raw.forEach(function(r){var o=_padRow(r);if(o)rows.push(o);});
      if(!rows.length){
        showToast('El Excel no tiene filas con empresa ni CUIT');
        renderPadronAdmin();return;
      }
      if(st)st.innerHTML='<span style="color:var(--gray)">Guardando '+rows.length+' empresas...</span>';
...
function _fgEnsurePadronBtn(){
  ...
  wrap.insertAdjacentHTML('beforeend',
    '<button type="button" id="fg-pad-btn" onclick="openPadronPop()" title="Buscar en el padrón (razón social o CUIT)">&#128269;</button>');
...
function openPadronPop(){
  ...
  var q=document.getElementById('pad-q');
  if(q){q.value=(document.getElementById('empresa')||{}).value||'';q.focus();q.select();}
  _padSel=0;
